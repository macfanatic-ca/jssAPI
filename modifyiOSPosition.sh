...
if [[ ${#} -lt 2 ]] || [[ ${*} == *"-h"* ]] || [[ ${*} == *"--help"* ]]; then
    ...
    echo "CSV should have 2 columns with the following headers:
        1. position         (student || teacher || staff) [required]
...
updateDeviceInfo() {
	putXML="<mobile_device><location><position>$position</position></location></mobile_device>"
...
# all the things
while IFS=, read position serialNumber
do
	if [[ -z $position ]] || [[ -z $serialNumber ]]; then
